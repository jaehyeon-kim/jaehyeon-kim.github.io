...
```{r kmeans_parallel, message=FALSE, warning=FALSE}
# parallel
split = detectCores()
eachStart = 25
...
```{r kmeans_foreach, message=FALSE, warning=FALSE}
# foreach
split = detectCores()
eachStart = 25
# set up iterators
...
```{r rf_parallel, message=FALSE, warning=FALSE}
## Random forest
...
```{r rf_foreach, message=FALSE, warning=FALSE}
# foreach
rm(list = ls())
set.seed(1237)
x = matrix(runif(500), 100)
...
```{r bgg_setup, message=FALSE, warning=FALSE}
## Bagging
rm(list = ls())
...
```{r bgg_combine, message=FALSE, warning=FALSE}
...
```{r bgg_parallel, message=FALSE, warning=FALSE}
# data
library(ISLR)
data(Carseats)
...
```{r bgg_foreach, message=FALSE, warning=FALSE}
# foreach
split = detectCores()
eachTrees = 250
# set up iterators
