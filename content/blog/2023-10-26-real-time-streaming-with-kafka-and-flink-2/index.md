---
title: Produce data to Kafka using Lambda - Real Time Streaming with Kafka and Flink Lab 1
date: 2023-10-26
draft: false
featured: false
comment: true
...
description: Produce fake taxi ride data into a Kafka topic on Amazon MSK from an AWS Lambda function invoked repeatedly by an EventBridge schedule rule.
---
...
![An EventBridge rule drives Kafka producer Lambda functions into Amazon MSK, which feeds Flink apps writing to OpenSearch and S3, a Camel connector writing to DynamoDB, and Kafka consumer Lambda functions](featured.png#center "Labs 1 to 6 built around an Amazon MSK cluster")

## Infrastructure

The infrastructure is created using [Terraform](https://developer.hashicorp.com/terraform) and the source can be found in the [**GitHub repository**](https://github.com/jaehyeon-kim/flink-demos/tree/master/real-time-streaming-aws) of this post.
...
![EventBridge rule crons-rule, enabled and scheduled, with five targets all pointing at the same kafka_producer Lambda function](cron-rule.png#center "Schedule rule with five producer targets")

### Monitor Topic

A Kafka management app can be a good companion for development as it helps monitor and manage resources on an easy-to-use user interface. We'll use [Kpow Community Edition](https://docs.factorhouse.io/kpow/getting-started) in this post, which allows you to link a single Kafka cluster, Kafka connect server and schema registry. Note that the community edition is valid for 12 months and the licence can be requested on this [page](https://factorhouse.io/products/kpow/). Once requested, the licence details will be emailed, and they can be added as an environment file (*env_file*).

The app needs additional configurations in environment variables because the Kafka cluster on Amazon MSK is authenticated by IAM - see [this page](https://docs.factorhouse.io/kpow/provider/amazon-msk/kafka-cluster) for details. The bootstrap server address can be found on AWS Console or executing the following Terraform command. 
...
![Kpow topic details for taxi-rides, 5 partitions holding about 540000 messages each and 2.7 million in total](kafka-topic.png#center "The taxi-rides topic in Kpow")

Also, we can inspect topic messages in the *Data* tab as shown below.

![Kpow data view of a taxi-rides record, key id5111704 and a value with vendor id, passenger count, trip duration and pickup and dropoff coordinates](topic-message.png#center "A single taxi ride message")
