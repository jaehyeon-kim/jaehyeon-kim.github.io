...
![Terminal with user recommendation lines and click results above Redis sink writer logs updating the model for each product](recommender-output.webp#center "Live Simulation Result")

You can inspect feedback events on Kafka UI at [http://localhost:8086](http://localhost:8086).

![Kafbat UI messages tab for the feedback-events topic showing a JSON record with product id, reward and context vector](feedback-events.png#center "Feedback Events")
