...
![IAM user Permissions tab listing three attached AWS managed policies](PM-user.png#center "Attached policies on the IAM user")


Unlike users (and groups that can have one or more users with relevant permissions), AWS services (eg AWS Lambda) can assume a *role*, inheriting the permissions given to the role. Roles don't have permanent credentials assigned to them but, when a service assumes a role, temporary credentials are assigned and they are used to authorize the service to do what's defined in the (user-defined) policies attached to the role. Note that temporary credentials are made up of *Access Key ID*, *Secret Access Key* and *Security Token* and are generated by [AWS Security Token Service (STS)](https://docs.aws.amazon.com/STS/latest/APIReference/Welcome.html). In order to assume a role, a service needs another type of policy called *trust policy*, which defines who can assume a role. Therefore we need both types of policy so as to deploy the POC application at AWS Lambda.
...
![Create Policy page offering Copy an AWS Managed Policy, Policy Generator and Create Your Own Policy](P01-create.png#center "Choosing Create Your Own Policy")
...
![Review Policy page with ServerlessPOC name and a valid s3:GetObject policy document](P03-review.png#center "Reviewing the ServerlessPOC policy")
...
![Set Role Name step with ServerlessPOC typed into the Role Name box](R01-name.png#center "Naming the role")

### Step 2 : Select Role Type

*AWS Lambda* is selected in the *AWS Service Roles* group.

![Select Role Type list with AWS Lambda highlighted among AWS Service Roles](R02-type.png#center "Selecting the AWS Lambda role type")
...
![Attach Policy step with the ServerlessPOC tick box checked among three filtered policies](R04-attach.png#center "Attaching policies to the role")

### Step 5 : Review

After reviewing, it is possible to create the role.

![Review step showing role name, ARN, trusted entity lambda.amazonaws.com and two attached policies](R05-review.png#center "Reviewing the role before creation")
...
![Role Permissions tab listing ServerlessPOC and AWSLambdaBasicExecutionRole as managed policies](PM-role.png#center "Managed policies on the created role")
...
![Select blueprint page with Blank Function chosen next to a Kinesis Firehose blueprint](L01-blueprint.png#center "Selecting the Blank Function blueprint")
...
![Configure triggers page with an empty dashed box beside the Lambda icon](L02-trigger.png#center "No trigger selected")
...
![Configure function form with name ServerlessPOCAdmission, Python 2.7 runtime and an S3 link URL](L03-configure-01.png#center "Naming the function and pointing to the package")
...
![Handler set to handler.lambda_handler, existing role ServerlessPOC, memory 128 MB and six second timeout](L03-configure-02.png#center "Handler, role, memory and timeout")

The remaining settings are left as they are. 

![Advanced settings left at defaults with no DLQ resource, No VPC and the default KMS key](L03-configure-03.png#center "Remaining settings kept as they are")
...
![Input test event editor holding gre 800, gpa 4 and rank 1](L04-save-and-test-01.png#center "Entering the test event")
...
![Execution result succeeded returning result true, with 658.93 ms duration and 75 MB memory used](L04-save-and-test-02.png#center "Test output and summary")
