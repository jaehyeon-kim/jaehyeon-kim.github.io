...
![kafka-ui connector page for order-source, state RUNNING with two tasks and none failed](source-connector.png#center "order-source connector details on kafka-ui")
...
![AWS Glue registry online-order listing two schemas, order and customer](schemas-01.png#center "Schemas of the two topics in the AWS Glue registry")
...
![kpow Schema page under registry glue1 showing customer and order subjects as AVRO version 1](schemas-02.png#center "Same two subjects seen on kpow under the glue1 registry")

### Kafka Topics

As configured, the source connector ingests messages to the *customer* and *order* topics.

![kafka-ui topic list with customer at 2204 messages and order at 2194, three partitions each](topics-01.png#center "Topics filled by the source connector")
...
![kafka-ui order topic messages with Value Serde set to online-order and one record expanded](topics-02.png#center "An order record deserialised with the Glue serializer plugin")
...
![kpow Data Inspect form with AVRO value deserializer and glue1 schema registry selected](topics-03-01.png#center "Choosing AVRO and the glue1 registry before searching on kpow")

![kpow search results showing deserialised customer and order records with partition, offset and value](topics-03-02.png#center "Deserialised messages from both topics on kpow")
...
![kafka-ui page for order-sink running the S3 sink connector class with two running tasks](sink-connector.png#center "order-sink connector details on kafka-ui")
...
![kafka-ui consumer group connect-order-sink, stable, two members, six partitions, total lag 343](consumer-01.png#center "Consumer group created by the sink connector")

### S3 Destination

The sink connector writes messages of the two topics (*customer* and *order*), and topic names are used as prefixes. 

![S3 console listing the topics prefix with two folders, customer and order](s3-01.png#center "Topic names used as prefixes in the S3 bucket")

As mentioned, the default partitioner prefixes files further by the partition number, and it can be checked below.

![S3 console listing three folders under order, partition=0, partition=1 and partition=2](s3-02.png#center "Default partitioner adds a folder per Kafka partition")
...
![S3 console listing Json files from order+1+0000000000 to order+1+0000000400, 6.2 KB each](s3-03.png#center "Json files named by topic, partition and start offset")
