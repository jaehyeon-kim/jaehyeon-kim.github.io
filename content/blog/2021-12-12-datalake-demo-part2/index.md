...
![Five numbered stages from Aurora PostgreSQL through MSK Connect and EMR to Athena and QuickSight](00-01-architecture.png#center "Data lake architecture, from database change events to the dashboard")
...
![SoftEther setup wizard with VPN Server Manager, admin tools only, boxed in red](vpn-00.png#center "Installing only the SoftEther VPN Server Manager admin tools")
...
![New Connection Setting dialog with setting name, host name and password boxed in red](vpn-01.png#center "Connection settings for the VPN server, port 443 in Server Admin Mode")
...
![VPN Client Manager dialog with host name, virtual hub, user name and password boxed in red](vpn-02.png#center "Client connection settings, with the connection listed as Connected")
...
![MSK create connector step 1, debezium-source-connector picked from two active custom plugins](connector-01.png#center "Step 1, choosing the Debezium source plugin")
...
![Connector properties step with the connector name set to orders-source and no description](connector-02-01.png#center "Step 2, naming the connector")

![MSK cluster list with cluster-jaehyeon-msk selected, IAM authentication and Kafka 2.8.1](connector-02-02.png#center "Choosing the MSK cluster and its IAM authentication")

![Configuration box holding 19 lines of Debezium PostgreSQL properties, zero errors and warnings](connector-02-03.png#center "Debezium source connector configuration settings")

![Provisioned capacity set to one MCU and one worker, totalling 4 GiB and up to 10 Gbps](connector-02-04.png#center "Connector capacity, provisioned with a single worker")

![Kafka Connect 2.7.1, the MSK default worker configuration and an MSKConnectRole IAM role](connector-02-05.png#center "Worker configuration and the connector IAM role")


In the third step, we configure cluster security. As we selected IAM Authentication, only TLS encryption is available.

![Security step showing IAM role-based access control and TLS encrypted traffic in transit](connector-03.png#center "Step 3, access control and encryption preset by the chosen cluster")

Finally, we can configure logging. A log group is created in the CloudFormation, and we can add its ARN.

![Logs step with CloudWatch Logs ticked and a log group ARN entered, S3 and Firehose unticked](connector-04.png#center "Step 4, sending connector logs to CloudWatch")
...
![Topic msk.datalake.cdc_events listed with one partition, replication factor 2 and 829 messages](kafka-ui-01.png#center "CDC topic in Kafka UI, holding 829 messages")
...
![Messages tab listing offsets 0 to 3 on partition 0 with JSON schema keys and contents](kafka-ui-02.png#center "Individual messages inside the CDC topic")
...
![Expanded message showing a 19 field schema, with a Copy to clipboard and Save as a file menu](kafka-ui-03.png#center "Message key, content and headers, with copy and save options")
...
![S3 object for order 11077 with two JSON lines, shipped_date null then 10392 boxed in red](output-file-update.png#center "Update event, shipped_date changes from null to 10392")
...
![S3 object for order 11075 with four JSON lines, the first holding null order_items and products](output-file-insert.png#center "Insert event, order items and products fill in over later records")
