...
In this post, we develop two Apache Beam pipelines that calculate average word lengths from input texts that are ingested by a Kafka topic. They obtain the static in different angles. The first one emits the global average lengths whenever a new input text arrives while the latter triggers those values in a sliding window. 
...
We have two Kafka text producers as many pipelines read/process text messages from a Kafka topic. The first producer sends fake text messages that are generated by the Faker package while the latter accepts text inputs by user, which is designed for controlling text messages as well as message creation timestamps. The latter feature is useful for testing late arrival data. See [Part 1](/blog/2024-07-04-beam-examples-1) for details about the Kafka producers.

## Beam Pipelines

We develop two Apache Beam pipelines that calculate average word lengths from input texts that are ingested by a Kafka topic. The first one emits the global average lengths whenever a new input text arrives while the latter triggers those values in a sliding window. 

### Shared Transforms

Both the pipelines read text messages from an input Kafka topic and write outputs to an output topic. Therefore, the data source and sink transforms are refactored into a utility module as shown below. Note that, the Kafka read and write methods has an argument called `deprecated_read`, which forces to use the legacy read when it is set to *True*. We will use the legacy read in this post to prevent from [this issue](https://github.com/apache/beam/issues/20979).
...
We have three elements and the pipeline should emit the average word length whenever a new element is delivered. Therefore, the expected average word length values become 1 (1/1), 1.5 (3/2) and 2 (6/3).
