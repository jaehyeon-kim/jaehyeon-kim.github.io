...
![Kafka Connect menu of kafka-ui showing the source connector and its tasks](source-connector.png#center "Kafka Connect menu of kafka-ui showing the source connector and its tasks")

### Kafka Topics

As configured, the source connector ingests messages to the *customer* and *order* topics.

![Topic list in kafka-ui with the customer and order topics fed by the connector](topics-01.png#center "Topic list in kafka-ui with the customer and order topics fed by the connector")

We can browse individual messages in the *Messages* tab of a topic.

![Messages tab of a topic in kafka-ui with individual generated records](topics-02.png#center "Messages tab of a topic in kafka-ui with individual generated records")
...
![Sink connector details in kafka-ui with its configuration and tasks](sink-connector.png#center "Sink connector details in kafka-ui with its configuration and tasks")
...
![Consumer connect-order-sink in the stable state with two members on both topics](consumer-01.png#center "Consumer connect-order-sink in the stable state with two members on both topics")

### S3 Destination

The sink connector writes messages of the two topics (*customer* and *order*), and topic names are used as prefixes. 

![S3 bucket with one prefix per topic, customer and order](s3-01.png#center "S3 bucket with one prefix per topic, customer and order")

As mentioned, the default partitioner prefixes files further by the partition number, and it can be checked below.

![S3 prefixes split further by the topic partition number](s3-02.png#center "S3 prefixes split further by the topic partition number")
...
![JSON files in S3 named by topic, partition and start offset](s3-03.png#center "JSON files in S3 named by topic, partition and start offset")
