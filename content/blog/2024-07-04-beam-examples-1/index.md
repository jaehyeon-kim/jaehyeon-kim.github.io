...
We have two Kafka text producers as many pipelines read/process text messages from a Kafka topic. The first producer sends fake text messages that are generated by the [Faker](https://faker.readthedocs.io/en/master/) package. We can run the producer simply by executing the producer script.
